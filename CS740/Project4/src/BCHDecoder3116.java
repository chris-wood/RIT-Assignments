public class BCHDecoder3116 
{
	...
	public BCHDecoder3116()
	{
        ...
        p[0] = p[2] = p[5] = 1; // caw - need a new irreducible polynomial
        p[1] = p[3] = p[4] = 0;
        InitializeDecoder();
	}
    ...
    private int[] calcSyndrom(int codeword)
    {
        ...
        for (int i = 0; i < 6; i++) // caw: syndrome is a vector with 1,2,...,2t elements
        {
            for (int j = 0; j < n; j++)
            {
                if (((codeword >> (j + 1)) & 1) != 0)
    ...
    {
        int[] S = new int[] { 0, 0, 0, 0, 0, 0 }; // caw: syndrome is 2t elements wide in the vector
        int s3 = 0;
        int[] C = new int[] { 0, 0, 0 };
