    ...
    (cond
      ((and (node? (get-node-l tree)) (node? (get-node-r tree))) ; if they're both nodes, choose which one has the deepest element
       (let ; to reduce typing!
           ((leftVal (deepest-helper (get-node-l tree)))
            (rightVal (deepest-helper (get-node-r tree))))
         (if (>= (cdr leftVal) (cdr rightVal)) ; choose the one that has the deepest element (with preference to the left)
             (cons (car leftVal) (+ 1 (cdr leftVal)))
             (cons (car rightVal) (+ 1 (cdr rightVal))))))
...
(define (btree-subtree? bt1 bt2)
  ...
  ; Invoke the subtree check function
...
(define btree-subtrees-test04
  (list "btree-subtrees-test04"
        (list btree-ex4)
        (list btree-ex4 
              (node (node (node leaf 1 leaf) 2 leaf) 3 (node leaf 5 leaf))
              (node (node (node leaf 1 leaf) 2 leaf) 3 leaf)
              (node (node leaf 1 leaf) 2 leaf)
              (node (node leaf 2 leaf) 3 (node (node leaf 4 leaf) 5 leaf))
              ...
              (node leaf 5 leaf)
              leaf)))
(define btree-subtrees-test05
  (list "btree-subtrees-test05"
        (list btree-ex5)
        (list btree-ex5
              (node (node (node leaf 1 (node leaf "a" leaf)) 2 leaf) 3 (node leaf 5 leaf))
              (node (node (node leaf 1 (node leaf "a" leaf)) 2 leaf) 3 leaf)
              (node (node (node leaf 1 leaf) 2 leaf) 3 (node (node leaf 4 leaf) 5 leaf))
              (node (node (node leaf 1 leaf) 2 leaf) 3 (node leaf 5 leaf))
              (node (node (node leaf 1 leaf) 2 leaf) 3 leaf)
              (node (node leaf 1 (node leaf "a" leaf)) 2 leaf)
              (node (node leaf 1 leaf) 2 leaf)
              (node (node leaf 2 leaf) 3 (node (node leaf 4 leaf) 5 leaf))
              (node (node leaf 2 leaf) 3 (node leaf 5 leaf))
              (node (node leaf 2 leaf) 3 leaf)
              ...
              (node leaf 5 leaf)
              leaf)))
(define btree-subtrees-test06
  (list "btree-subtrees-test06"
        (list btree-ex6)
        (list btree-ex6
              (node (node (node (node leaf "b" leaf) "c" leaf) 4 leaf) 5 leaf)
              (node (node (node leaf "b" leaf) "c" leaf) 4 leaf)
              (node (node (node leaf "c" leaf) 4 leaf) 5 leaf)
              (node (node (node leaf 1 (node leaf "a" leaf)) 2 leaf) 3 (node (node (node leaf "c" leaf) 4 leaf) 5 leaf))
              (node (node (node leaf 1 (node leaf "a" leaf)) 2 leaf) 3 (node (node leaf 4 leaf) 5 leaf))
              (node (node (node leaf 1 (node leaf "a" leaf)) 2 leaf) 3 (node leaf 5 leaf))
              (node (node (node leaf 1 (node leaf "a" leaf)) 2 leaf) 3 leaf)
              (node (node (node leaf 1 leaf) 2 leaf) 3 (node (node (node (node leaf "b" leaf) "c" leaf) 4 leaf) 5 leaf))
              (node (node (node leaf 1 leaf) 2 leaf) 3 (node (node (node leaf "c" leaf) 4 leaf) 5 leaf))
              (node (node (node leaf 1 leaf) 2 leaf) 3 (node (node leaf 4 leaf) 5 leaf))
              (node (node (node leaf 1 leaf) 2 leaf) 3 (node leaf 5 leaf))
              (node (node (node leaf 1 leaf) 2 leaf) 3 leaf)
              (node (node leaf "b" leaf) "c" leaf)
              (node (node leaf "c" leaf) 4 leaf)
              (node (node leaf 1 (node leaf "a" leaf)) 2 leaf)
              (node (node leaf 1 leaf) 2 leaf)
              (node (node leaf 2 leaf) 3 (node (node (node (node leaf "b" leaf) "c" leaf) 4 leaf) 5 leaf))
              (node (node leaf 2 leaf) 3 (node (node (node leaf "c" leaf) 4 leaf) 5 leaf))
              (node (node leaf 2 leaf) 3 (node (node leaf 4 leaf) 5 leaf))
              (node (node leaf 2 leaf) 3 (node leaf 5 leaf))
              (node (node leaf 2 leaf) 3 leaf)
              (node (node leaf 4 leaf) 5 leaf)
              ...
              (node leaf 1 leaf)
              (node leaf 2 leaf)
              (node leaf 3 (node (node (node (node leaf "b" leaf) "c" leaf) 4 leaf) 5 leaf))
              (node leaf 3 (node (node (node leaf "c" leaf) 4 leaf) 5 leaf))
              (node leaf 3 (node (node leaf 4 leaf) 5 leaf))
              (node leaf 3 (node leaf 5 leaf))
              (node leaf 3 leaf)
              (node leaf 4 leaf)
...
(define btree-subtrees-tests
  (list btree-subtrees-test01
        ...
        btree-subtrees-test05
        btree-subtrees-test06))
...
(define (sbtree-find sbt e)
  ; Helper function that simply traverss the sbtree using the comp comparison
  (define (sbtree-find-helper bt comp e)
    (cond 
      ((null? bt) #f)
      ((leaf? bt) #f)
      (else 
       (if (equal? e (get-node-x bt)) 
            ...
            (if (comp e (get-node-x bt)) ; run the comparison and check the right branch
                (sbtree-find-helper (get-node-l bt) comp e)
                (sbtree-find-helper (get-node-r bt) comp e))))))
  
  ; Extract the comparison operator and tree and call the new find function
  (sbtree-find-helper (get-3rd sbt) (get-2nd sbt) e)) 
...
(define (sbtree-insert sbt e)
  ; Helper function to traverse the tree using insert and check for the place to insert it
  (define (sbtree-insert-helper bt comp e)
    (cond 
      ...
      (else 
       (if (equal? e (get-node-x bt)) 
           ...
           (if (comp e (get-node-x bt)) ; run the comparison and check the correct branch
               (node (sbtree-insert-helper (get-node-l bt) comp e) (get-node-x bt) (get-node-r bt))
               (node (get-node-l bt) (get-node-x bt) (sbtree-insert-helper (get-node-r bt) comp e)))))))
  
  ; Extract the comparison operator and tree and call the new insert function
  ; Then build up the resulting sbtree when done...
  (list 'sbtree (get-2nd sbt) (sbtree-insert-helper (get-3rd sbt) (get-2nd sbt) e))) 
