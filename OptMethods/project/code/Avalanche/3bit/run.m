...
IntVars   = logical([ 1 1 1 1 ]); % Integer variables: x(1)-x(4)
VarWeight = []; % whatever

% There are divisions and square roots involving x(2), so we must
% have a small but positive value for the lower bound on x(2).
upperBound = 2^bits - 1;
x_L = [ 0 0 0 0 ]'; % Lower bounds on x
x_U = [ upperBound upperBound upperBound upperBound ]'; % Upper bounds on x

% Initailize the design variables (all zeros in the S-box)
x_0 = [0 1 2 2]; % relatively sparse values

x_min = [ 0 0 0 0 ]; % Used for plotting, lower bounds
x_max = [ upperBound upperBound upperBound upperBound ]; % Used for plotting, upper bounds

HessPattern = spalloc(SBOX_SIZE, SBOX_SIZE, 0); % All elements in Hessian are zero.
ConsPattern = [ 0 1 0 0 ; ... % Sparsity pattern of nonlinear
                1 0 1 0 ]; % constraint gradient
...
Prob = minlpAssign('avalanche_f', 'avalanche_g', 'avalanche_h', HessPattern, ...
