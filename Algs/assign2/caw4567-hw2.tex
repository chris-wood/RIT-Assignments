...
The solution to $T_{F}(n)$ can be solved using the method of homogeneous equations, which yields the result that $T_{F}(n) = \frac{1}{\sqrt{5}}(\phi^{n} - \phi'^{n})$, where $\phi = \frac{1 + \sqrt{5}}{2}$ and $\phi' = \frac{1 - \sqrt{5}}{2}$. Thus, we can express this solution, and subsequently the time complexity of $F_{n}$, as a function of exponential growth of $n$, as shown below. 
\begin{eqnarray*}
